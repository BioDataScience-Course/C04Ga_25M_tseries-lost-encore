...
read("./data/mobility_wallonia.rds") -> mob_wal
```

```{r Exploration des données}
plot(mob_wal)
summary(mob_wal)
skimr::skim(mob_wal)
```

```{r Convesion en objet ts}
mob_wal_ts <- ts(mob_wal, start = c(2020, 46), end = c(2022, 288), frequency = 365)
class(mob_wal_ts)
```
...
mobw_ts <- ts(mobw$transit_stations_percent_change_from_baseline, start = c(2020, 46), frequency = 365)
