---
--- title: "___" author: "___" format:   html:     code-fold: true     code-tools: true     toc: true editor: visual lang: fr ---
---
...
if (!"tools:tests" %in% search())   source(here::here("tests/tools_tests.R"), attach(NULL, name = "tools:tests")) 
...
```{r extraction des résidus } mts_loisir <- tseries(loisir_loess) plot(mts_loisir)  mts_loisir_w<- tseries(loisir_loess_w) plot(mts_loisir_w)  tseries(loisir_loess_WE)->mts_loisir_WE plot(mts_loisir_WE)}
```
